...
def plotly_dark_layout(fig, height=260, margin=None):
    """Apply consistent dark plotly theme."""
...
def route_class(route):
    m = {
        ...
        "Investigation Flag": "route-fraud",
        "Specialist Queue":   "route-spec",
    }
    return m.get(route, "route-manual")
...
with st.sidebar:
    ...
    <div style="padding:12px 20px;">
      <div style="font-size:10px;color:rgba(226,244,255,0.35);letter-spacing:1px;
                  text-transform:uppercase;margin-bottom:6px;">System Status</div>
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
        <div style="width:6px;height:6px;border-radius:50%;background:#00ff9d;
                    box-shadow:0 0 8px #00ff9d;"></div>
      ...
      <div style="display:flex;align-items:center;gap:8px;">
        <div style="width:6px;height:6px;border-radius:50%;background:#00f5ff;
                    box-shadow:0 0 8px #00f5ff;animation:blink 1.5s ease infinite;"></div>
...
if st.session_state.page == "Dashboard":
    ...
    <div class="hero-section">
      <div class="auto-badge">
        <div class="auto-dot"></div> Autonomous Mode · Active
      </div>
      <div class="hero-h1">Claims Intelligence</div>
      <div class="hero-h2">at machine speed.</div>
      <div class="hero-sub">
        NEXORA ingests FNOL documents, extracts fields, validates, scores fraud
        risk and routes — fully explainable, in seconds.
      </div>

      <!-- Animated stats strip -->
      <div style="display:flex;gap:24px;flex-wrap:wrap;margin-bottom:24px;animation:fadeSlideUp 0.8s ease 0.5s both;">
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:6px;height:6px;border-radius:50%;background:#00ff9d;box-shadow:0 0 8px #00ff9d;animation:blink 1.4s ease infinite;"></div>
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:6px;height:6px;border-radius:50%;background:#00ff9d;box-shadow:0 0 8px #00ff9d;animation:blink 1.4s ease 0.3s infinite;"></div>
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:6px;height:6px;border-radius:50%;background:#00f5ff;box-shadow:0 0 8px #00f5ff;animation:blink 1.4s ease 0.6s infinite;"></div>
    ...
    # Hero action buttons via Streamlit
    ...
    for i, (col, (label, val, vc, delta, up, icon, bar_pct)) in enumerate(zip([k1,k2,k3,k4], kpi_data)):
        with col:
            ...
            st.markdown(f"""
            <div class="kpi-card" style="animation:fadeSlideUp 0.6s ease {0.05*i:.2f}s both;">
              <div class="kpi-icon">{icon}</div>
              <div class="kpi-label">{label}</div>
              <div class="kpi-value {vc}">{val}</div>
              <div class="kpi-delta {delta_cls}">{delta}
                <span class="kpi-delta-val">vs last week</span>
              </div>
              <div style="height:3px;background:rgba(255,255,255,0.06);border-radius:2px;margin-top:14px;overflow:hidden;">
                <div style="height:100%;width:{bar_pct}%;background:{bar_color};border-radius:2px;
                  animation:barFill 1.2s cubic-bezier(0.4,0,0.2,1) {0.3+0.1*i:.2f}s both;
    ...
    with left_col:
        ...
        <div class="card-header">
          ...
          <div>
            <div class="card-title">FNOL Intake</div>
            <div class="card-sub">Paste text or upload .txt</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with right_col:
        ...
        <div class="card-header">
          ...
          <div>
            ...
            <div class="card-sub">extractedFields · missingFields · route · reasoning</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.last_result:
            ...
            qc     = quality_color(validation.get("quality",""))

            result_placeholder.markdown(f"""
            <div style="animation:fadeSlideUp 0.5s ease;">
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
                <span class="route-badge {rc_cls}">{route}</span>
                <span class="status-pill {({'high':'pill-ok','medium':'pill-warn','low':'pill-err'}).get(validation.get('quality',''),'pill-warn')}">
                  Quality: {validation.get('quality','—').upper()}
                </span>
                <span style="font-size:11px;color:var(--text-dim);margin-left:auto;">
                  Score: <span class="mono cyan">{validation.get('confidence_score',0)}</span>
                </span>
              </div>

              <div class="field-grid">
                {"".join(f'<div class="field-item"><div class="field-key">{k.replace("_"," ")}</div><div class="field-val">{str(v)[:28] if v else "—"}</div></div>'
                         for k,v in list(data.items())[:8] if not k.startswith("_"))}
              </div>

              <div style="margin-top:12px;padding:10px 14px;background:rgba(0,245,255,0.04);
                          border:1px solid var(--border2);border-radius:8px;">
                <div style="font-size:10px;color:var(--text-dim);letter-spacing:1px;
                            text-transform:uppercase;margin-bottom:4px;">Reasoning</div>
                <div style="font-size:12px;color:var(--text);">{reason}</div>
              </div>

              {"".join([f'<div class="fraud-alert" style="margin-top:10px;">🚨 {x.replace("_"," ").title()}</div>'
                        for x in validation.get("inconsistent",[])[:2]])}
            </div>
            """, unsafe_allow_html=True)
        else:
            result_placeholder.markdown("""
            <div class="decision-empty" style="min-height:240px;display:flex;
                flex-direction:column;align-items:center;justify-content:center;gap:12px;">
              <div style="font-size:36px;opacity:0.3;animation:spin 8s linear infinite;">✦</div>
              <div style="font-size:13px;color:var(--text-dim);text-align:center;">
                Submit an FNOL to see the agent's structured<br>extraction, validation and routing decision.
              </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    if process_btn and fnol_text.strip():
        with st.spinner(""):
            prog = st.progress(0)
            msgs = [
                "Preprocessing FNOL document…",
                "Extracting structured fields via AI…",
                "Validating policy dates & damage…",
                "Scoring fraud indicators…",
                "Routing claim decision…",
            ]
            status_box = st.empty()
            for i, msg in enumerate(msgs):
                status_box.markdown(f"""
                <div class="glass-card" style="padding:12px 16px;margin-top:8px;">
                  <span class="cyan mono" style="font-size:12px;">▶ {msg}</span>
                </div>
                """, unsafe_allow_html=True)
            ...
            st.session_state.last_result = dict(
                ...
                route=route, reason=reason, risk=risk
            )
    ...
    # ── THROUGHPUT CHART + AI CONSOLE ──────────────────────────────────────────
    tc, ac = st.columns([1.4, 1])

    with tc:
        st.markdown("""
        <div class="section-header">
          <div class="section-title">Throughput · 12h</div>
          <div style="font-size:12px;color:var(--text-dim);">Claims processed per hour</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        noise = [b + random.randint(-4,4) for b in base]
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate="%{x}<br><b>%{y}</b> claims<extra></extra>",
        ))
    ...
    with ac:
        ...
        <div class="ai-console" style="min-height:280px;">
          <div class="ai-console-header">
            <div class="ai-console-dot"></div>
            <div class="ai-console-title">AI Console</div>
            <div class="ai-console-sub">GPT-4o · Claims v2</div>
          </div>
        ...
        if st.button("Open full AI Console →", key="dash_open_console"):
            st.session_state.page = "AI Console"
            st.rerun()

    st.markdown("<br>", unsafe_allow_html=True)

    # ── LIVE CLAIMS PIPELINE TABLE ─────────────────────────────────────────────
    st.markdown("""
    <div class="section-header">
      <div class="section-title">Live Claims Pipeline</div>
    ...
    if rows:
        ...
        headers = ["ID", "Policy", "Damage", "Risk", "Route", "Date"]
        for col, h in zip(cols_h, headers):
            col.markdown(f"<div style='font-size:10px;font-weight:700;color:var(--text-dim);"
                         f"letter-spacing:1px;text-transform:uppercase;padding:6px 0;'>{h}</div>",
                         unsafe_allow_html=True)
        ...
        for row in rows[-8:][::-1]:
            ...
            cols_r[0].markdown(f"<span class='mono cyan' style='font-size:11px;'>#{cid:04d}</span>",
                               unsafe_allow_html=True)
            cols_r[1].markdown(f"<span style='font-size:12px;'>{str(policy)[:14]}</span>",
                               unsafe_allow_html=True)
            cols_r[2].markdown(f"<span style='font-size:12px;font-weight:600;'>"
                               f"₹{damage}</span>", unsafe_allow_html=True)
            rcolor = "#ff4d4d" if int(risk_ or 0) > 60 else "#ffd93d" if int(risk_ or 0) > 35 else "#00ff9d"
            cols_r[3].markdown(f"<span style='font-size:12px;color:{rcolor};font-weight:700;'>"
                               f"{risk_}</span>", unsafe_allow_html=True)
            cols_r[4].markdown(f"<span class='route-badge {rc_cls}' style='font-size:10px;"
                               f"padding:3px 8px;'>{route_}</span>", unsafe_allow_html=True)
            cols_r[5].markdown(f"<span style='font-size:10px;color:var(--text-dim);'>"
                               f"{datetime.now().strftime('%d %b %H:%M')}</span>",
                               unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
    else:
        st.markdown("""
        <div class="glass-card" style="text-align:center;padding:32px;color:var(--text-dim);font-size:13px;">
          No claims processed yet. Use the FNOL Intake above to get started.
        </div>
        """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="hero-sub" style="margin-bottom:24px;">
      Upload or paste any FNOL document. NEXORA extracts, validates and routes it instantly.
    </div>
    """, unsafe_allow_html=True)
    ...
    with lc:
        ...
        <div class="card-header">
          <div class="card-icon">◈</div>
          <div>
            <div class="card-title">FNOL Document</div>
            <div class="card-sub">Paste raw text or upload a file</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with rc:
        ...
        <div class="card-header">
          <div class="card-icon">✦</div>
          <div>
            <div class="card-title">AI Extraction Result</div>
            <div class="card-sub">Structured fields · Validation · Routing</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if run_btn and claim_text.strip():
            bar = st.progress(0)
            for step, pct in [("Classifying document…",20), ("Extracting fields…",45),
                               ("Validating data…",65), ("Scoring risk…",82), ("Routing…",100)]:
                bar.progress(pct)
                result_area.markdown(f"""
                <div class="glass-card" style="padding:14px 18px;">
                  <span class="cyan mono" style="font-size:12px;">▶ {step}</span>
                </div>
                """, unsafe_allow_html=True)
                time.sleep(0.35)
            ...
            if data.get("_meta"):
                ...
                <div class="fraud-alert">
                  ⚠ Document classified as: <b>{data['_meta'].replace('_',' ').upper()}</b>
                </div>
                """, unsafe_allow_html=True)
            else:
                rc_cls = route_class(route)
                ...
                fields_html = "".join(
                    f'<div class="field-item"><div class="field-key">{k.replace("_"," ")}</div>'
                    f'<div class="field-val">{str(v)[:32] if v else "—"}</div></div>'
                    for k,v in data.items() if not k.startswith("_")
                )

                missing_html = "".join(
                    f'<span class="status-pill pill-err">{f}</span>'
                    for f in validation.get("missing",[])
                )
                inconsist_html = "".join(
                    f'<span class="status-pill pill-warn">{w.replace("_"," ")}</span>'
                    for w in validation.get("inconsistent",[])
                )
                ...
                <div style="animation:fadeSlideUp 0.5s ease;">
                  ...
                  <div style="margin-top:14px;padding:12px 16px;background:rgba(0,245,255,0.04);
                              border:1px solid var(--border2);border-radius:8px;">
                    <div style="font-size:10px;color:var(--text-dim);letter-spacing:1px;
                                text-transform:uppercase;margin-bottom:4px;">AI Reasoning</div>
                    <div style="font-size:12px;color:var(--text);">{reason}</div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            <div class="glass-card">
              ...
              <div style="font-size:12px;color:var(--text-dim);margin-bottom:8px;">
                Last processed result. Run a new FNOL to refresh.
              </div>
            ...
            <div class="decision-empty" style="min-height:300px;display:flex;
                flex-direction:column;align-items:center;justify-content:center;gap:12px;">
              <div style="font-size:48px;opacity:0.2;">✦</div>
              <div style="font-size:13px;color:var(--text-dim);text-align:center;">
                Run the AI agent to see extraction results here.
              </div>
            </div>
            """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="hero-sub" style="margin-bottom:24px;">
      Real-time fraud signal monitoring, geographic hotspot analysis, and risk scoring.
    </div>
    """, unsafe_allow_html=True)
    ...
    for col, (label, val, vc, delta, up) in zip([fk1,fk2,fk3,fk4], fkpis):
        with col:
    ...
    with mc:
        st.markdown("""
        <div class="section-header">
          <div class="section-title">Fraud Hotspot Map</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        fig.add_trace(go.Scattergeo(
            lat=lats, lon=lons, text=cities,
            mode="markers",
            marker=dict(
                size=sizes,
                color=sizes,
                colorscale=[[0,"rgba(0,255,157,0.7)"],[0.5,"rgba(255,211,61,0.8)"],[1,"rgba(255,77,77,0.9)"]],
                showscale=False,
        ...
        fig.update_geos(
            ...
            showland=True, landcolor="rgba(0,20,30,0.8)",
            showocean=True, oceancolor="rgba(0,5,15,0.9)",
            showcoastlines=True, coastlinecolor="rgba(0,245,255,0.2)",
            showframe=False,
        )
    ...
    with rc:
        ...
        values = [42, 35, 23]
        colors = ["rgba(0,255,157,0.8)", "rgba(255,211,61,0.8)", "rgba(255,77,77,0.8)"]

        fig = go.Figure(data=[go.Pie(
            labels=labels, values=values,
            hole=0.65,
            marker=dict(colors=colors, line=dict(color="rgba(0,0,0,0.3)", width=2)),
            textfont=dict(size=11, color="#e2f4ff"),
            hovertemplate="<b>%{label}</b><br>%{percent}<extra></extra>",
        )])
        fig.add_annotation(
            text="34<br><span style='font-size:10px'>flags</span>",
            x=0.5, y=0.5, showarrow=False,
            font=dict(size=22, color="#00f5ff"),
            align="center",
        )
        ...
        st.markdown("""
        <div class="fraud-alert">🚨 High Risk Claim Detected — FNOL-8801 · $22,150 · Theft</div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="section-header">
      <div class="section-title">Multi-Signal Fraud Radar</div>
      <div class="section-action">Run deep scan →</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if rows:
        ...
        with ri:
            ...
            for rw in risky[:3]:
                ...
                fig.update_layout(
                    polar=dict(
                        radialaxis=dict(visible=True, range=[0,100], tickfont=dict(size=8, color="#aaa")),
                        angularaxis=dict(tickfont=dict(size=10, color="#e2f4ff")),
                        bgcolor="rgba(0,0,0,0)",
                    ),
                    paper_bgcolor="rgba(0,0,0,0)",
                    font=dict(color="#e2f4ff"),
                    height=200, margin=dict(l=30,r=30,t=20,b=20),
        ...
        st.markdown("""
        <div class="glass-card" style="text-align:center;padding:32px;color:var(--text-dim);font-size:13px;">
          Process claims to see fraud radar analysis.
        </div>
        """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="hero-sub" style="margin-bottom:24px;">
      All processed claims with routing decisions, risk scores, and status tracking.
    </div>
    """, unsafe_allow_html=True)
    ...
    for col, label, val, cls in [
        (pa, "TOTAL CLAIMS",  total,  "val-ok"),
        (pb, "FAST-TRACK",    fast,   "val-ok"),
        (pc_, "MANUAL REVIEW", manual, "val-warn"),
        (pd, "FLAGGED",       fraud+spec, "val-err"),
    ]:
        with col:
            st.markdown(f"""
            <div class="kpi-card">
    ...
    with sc_:
        route_filter = st.selectbox("Filter by Route", ["All Routes", "Fast-track", "Manual Review", "Investigation Flag", "Specialist Queue"], key="pipeline_filter", label_visibility="collapsed")
    ...
    if rows:
        ...
        for col, h in zip(hcols, ["#", "Policy", "Claimant", "Damage", "Risk", "Route", "Quality"]):
            col.markdown(f"<div style='font-size:10px;font-weight:700;color:var(--text-dim);"
                         f"letter-spacing:1px;text-transform:uppercase;padding:4px 0;'>{h}</div>",
                         unsafe_allow_html=True)
        ...
        for row in rows[::-1]:
            ...
            try:
                ...
                d = {}
            try:
                v = json.loads(raw_val)
            except Exception:
                v = {}

            # Filters
            if route_filter != "All Routes" and route_ != route_filter:
                continue
            policy_   = str(d.get("policy_number",  f"PN-{cid:04d}"))
            claimant_ = str(d.get("claimant",        "—"))
            if search and search.lower() not in (policy_+claimant_).lower():
                continue
        ...
        st.markdown("""
        <div class="glass-card" style="text-align:center;padding:48px;color:var(--text-dim);font-size:13px;">
          No claims in the pipeline yet. Process an FNOL on the Claims page.
        </div>
        """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="hero-sub" style="margin-bottom:24px;">
      Chat directly with the claims AI. Ask about fraud signals, validate policies, or query claim data.
    </div>
    """, unsafe_allow_html=True)
    ...
    with chat_col:
        ...
        <div class="ai-console" style="min-height:400px;">
          <div class="ai-console-header">
            <div class="ai-console-dot"></div>
            <div class="ai-console-title">NEXORA · Claims Intelligence v2</div>
            <div class="ai-console-sub">GPT-4o-mini</div>
          </div>
        ...
        if not st.session_state.chat_history:
            st.markdown("""
            <div class="ai-msg">
              👋 NEXORA online. I can help you analyze claims, detect fraud signals,
              validate policy data, or summarize case files. What would you like to investigate?
            </div>
            """, unsafe_allow_html=True)
        ...
        user_input = st.text_input(
            "Ask NEXORA",
            placeholder="Ask NEXORA anything about your claims…",
            key="ai_input",
            label_visibility="collapsed",
        )
        ...
        <div style="margin-top:12px;">
          <div style="font-size:10px;color:var(--text-dim);letter-spacing:1px;
                      text-transform:uppercase;margin-bottom:8px;">Quick Prompts</div>
        </div>
        """, unsafe_allow_html=True)

        prompts = [
            "Summarize the top fraud risk factors in recent claims",
            "What's the most common reason for Manual Review routing?",
            "How is fraud score calculated in NEXORA?",
        ]
        for p in prompts:
    ...
    with info_col:
        ...
        capabilities = [
            ("🔍", "FNOL Analysis", "Extract & validate claim fields from raw documents"),
            ("⚡", "Fraud Detection", "Score risk signals and flag suspicious patterns"),
            ("◈", "Policy Lookup", "Cross-reference policy dates, limits and coverage"),
            ("✦", "Routing Logic", "Explain why a claim was routed to a specific queue"),
            ("◎", "Case Summary", "Summarize entire claim history and decisions"),
        ]
        ...
        <div class="glass-card">
          ...
          <div style="display:flex;flex-direction:column;gap:10px;">
            <div class="field-item">
              <div class="field-key">Claims Processed</div>
              <div class="field-val">{len(rows)}</div>
            </div>
            <div class="field-item">
              <div class="field-key">Chat Turns</div>
              <div class="field-val">{len(st.session_state.chat_history)}</div>
            </div>
            <div class="field-item">
              <div class="field-key">AI Model</div>
              <div class="field-val">GPT-4o-mini</div>
            </div>
            <div class="field-item">
              <div class="field-key">Agents Online</div>
              <div class="field-val">{st.session_state.agents_online}</div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="hero-sub" style="margin-bottom:24px;">
      Configure NEXORA AI models, thresholds, routing rules, and system preferences.
    </div>
    """, unsafe_allow_html=True)
    ...
    with s1:
        ...
        <div class="card-header">
          <div class="card-icon">⚡</div>
          <div><div class="card-title">AI Model Config</div></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.slider("Fraud Sensitivity", 0, 100, 65, 5, key="s_fraud_sens",
                  help="Higher = more claims flagged for investigation")

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("""
        <div class="card-header">
          <div class="card-icon">◈</div>
          <div><div class="card-title">Routing Thresholds</div></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with s2:
        ...
        <div class="card-header">
          <div class="card-icon">◉</div>
          <div><div class="card-title">System Preferences</div></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="card-header">
          <div class="card-icon">⊞</div>
          <div><div class="card-title">Database</div></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="glass-card" style="padding:16px;">
          ...
          <div style="font-size:10px;color:var(--text-dim);margin-top:6px;">
            {min(100,len(rows)*10)}% of 10-record preview capacity
          </div>
        </div>
        """, unsafe_allow_html=True)
